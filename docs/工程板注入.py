...
# **手寫的成員表**：新計畫不會自動進來。2026-08-28 加 01C 時漏了它，
# 於是 Phase A 顯示 49 tasks 而同頁下面說 01C 四格全交付——**同一塊板自相矛盾**。
# 下面的 `階段涵蓋全部計畫()` 是那次的產物：少一份就紅。
階段成員 = {
    "Phase A": ["01", "01B", "01C", "01D", "02", "03", "04"],
...
def 換頁首(文: str, 統: dict) -> str:
    """頁首那一行。"""
    return 換一次(
        r'(<span class="sub">)\d+ 子系統 · \d+ 任務 · 宣告的檔都在 \d+／\d+ · 生成自 ([^@]*)@ \w+',
        rf"\g<1>{統['計畫']} 子系統 · {統['task']} 任務 · "
        rf"宣告的檔都在 {統['交付']}／{統['task']} · 生成自 \g<2>@ {來源摘要()}",
        文,
        "頁首",
    )


def 階段涵蓋全部計畫(表: dict) -> None:
    """四個 Phase 的成員必須恰好涵蓋全部計畫——**少一份就紅**。

    這條擋的是「加了新計畫卻忘記放進階段表」：那會讓 Phase 小計與
    計畫卡對不上，而兩邊都在同一塊板上。
    """
    列了 = {x for 們 in 階段成員.values() for x in 們}
    全部 = set(表)
    if 列了 != 全部:
        raise SystemExit(
            f"階段成員表與計畫集合不符——漏列 {sorted(全部 - 列了)}、"
            f"多列 {sorted(列了 - 全部)}；拒絕注入"
        )
...
def 換統計卡(文: str, 統: dict) -> str:
    ...
    for 值, 標 in [
        ...
        (統["步"], "步驟"),
        (統["待建"], "尚未建立"),
        # **不叫「已交付」**：這個數字只證明宣告的 Create 檔都在，不證明驗收過
        # ——真正的驗收是 ClaimSpec 閘，而它至今零執行（01 Task 12 未接線）。
        # sol 2026-08-28：檔案存在也是一種自報，只是報的人是檔案系統。
        (統["交付"], "宣告的檔都在"),
        (統["有負控段"], "有負控段的任務"),
        (統["claim"], "實存 claim 檔"),
        (統["未遷移"], "落點未遷移"),
