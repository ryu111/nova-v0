...
def i14_步驟序號連續(檔):
    """每個 task 的 `- [ ] **Step N` 序號必須恰好是 1..N 連續。

    存在理由（2026-08-27 實際發作）：把 01 的 `Step 5: Commit` 改成 `Step 6` 時
    做了全文替換，**七個 task 一起被改壞**——而當時沒有任何不變式管 Step 序號。
    I6 只管 task 標題的序號與位置，I13 只管 `Task N` 的文字引用，
    兩條都看不見 Step。

    **跳號的實害不是難看，是節奏斷掉**：刪掉的那步若是「先跑到指定的失敗」那一步，
    整個 task 就從「先看它紅」退化成「直接寫實作」，而帳面完全正常。
    CLAUDE.md 明定沒看過它紅過就不知道它在測什麼——這條不變式是那句話的執法器。

    立案時 sol、fable、claude 三票通過，但**在 repo 裡一個字元都不存在**，
    只活在對話裡達一天。sol 第三十七輪逐字：「在執法器、文件與負控落地以前，
    它只能算待議規則，不能列為已存在的機制。」這支就是那個落地。

    上線時全 22 份 194 個 task **零違規**，所以不帶基線債——
