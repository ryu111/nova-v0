...
**補殺手（P7，sol 2026-08-28 APPROVE）**：468 行的 offer-forgery 控制
從逐名兩個（`PROC_HIDE`、`NETWORK_DENY_ENFORCED`）改為**遍歷
`HOSTILE_CAPABILITIES` 集合的四個成員** parametrize——offer 對集合任一成員
宣告而無對應 probe evidence，admission 必紅 `hostile_capability_requires_probe`。
四列逐名列出：`PROC_HIDE`、`NETWORK_DENY_ENFORCED`、`UID_SEPARATION`、`MICROVM`。
**為什麼**：`UID_SEPARATION` 與 `MICROVM` 原本沒有殺手，而它們**會改變接受集合**
——散文寫「不可宣稱」不是機制。改成走集合之後，成員增減自動涵蓋。
