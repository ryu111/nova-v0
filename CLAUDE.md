...
**「跑了才擋」不等於「會被自動擋下」。** 目前沒有 git hook、沒有 CI、沒有
pre-commit、沒有 Claude hook 會自動跑這兩支——查證日 2026-08-27。所以仍然不准
寫「會被自動擋下」；能寫的是「`架構/檢查工程規範.py` 跑起來會 exit 1」。
哪天真的接上自動觸發，先確認那個觸發器存在且有測試背書，再改這一段。
