...
要當下的數字就跑 `uv run python docs/計畫複驗.py`，它會印出來，而且全部不變式同時驗過（項數也由它印，本檔不寫死）。
...
**閘已經接上自動執行點（2026-08-27，計畫 01 Task 14）。** 這一段原本寫「目前沒有任何
...
4. **CI 跑的是候選者自己那一份 checker。這是機制漏洞，不需要任何人為決定。**
   ...
   **全部來自 PR 的 commit**，沒有任何一道去 base branch 讀基準。
   ...
   讓閘的觸發與程式離開被閘管的人的寫入範圍。**尚未做，計畫 01 Task 17。**
